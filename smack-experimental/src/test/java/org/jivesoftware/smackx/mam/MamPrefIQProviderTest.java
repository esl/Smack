...
public class MamPrefIQProviderTest extends MamTest {
    ...
    public void checkMamPrefResult() throws Exception {
        ...
        MamPrefsExtension mamPrefsExtension = MamPrefsExtension.from(mamPrefsResultIQ);
        List<String> alwaysJids = mamPrefsExtension.getAlwaysJids();
        List<String> neverJids = mamPrefsExtension.getNeverJids();

        Assert.assertEquals(alwaysJids.size(), 1);
        Assert.assertEquals(neverJids.size(), 2);
        Assert.assertEquals(alwaysJids.get(0), "romeo@montague.lit");
        Assert.assertEquals(neverJids.get(1), "montague@montague.lit");
    }
